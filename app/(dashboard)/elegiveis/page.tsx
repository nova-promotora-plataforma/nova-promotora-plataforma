...
function buildVars(p: Partner) {
  ...
  const v3 = p.tempoLabel                                      // {{3}} situação atual
...
function buildCSV(partners: Partner[], suffix: string): void {
  ...
  const header = ['codigo', 'telefone_principal', ...telCols, '{{1}} nome', '{{2}} media_mensal', '{{3}} situacao', '{{4}} convenio', 'cidade', 'uf']
...
export default function ElegiveisPage() {
  ...
  function exportCSV() {
    ...
    buildCSV(base, `${inatividade}_${producao}`)
  }
  ...
  return (
    <>
      <TopBar title="Inativos Elegíveis — Sem Débito" />
      <main className="flex-1 overflow-auto p-5 space-y-5">
        ...
        <div className="rounded-md border border-[var(--nova-border)] bg-[var(--nova-bg-elev)] p-5 space-y-5">
          <div className="flex items-center gap-2 mb-1">
            <p className="text-sm font-semibold text-[var(--nova-text)]">Parceiros inativos elegíveis — sem débito</p>
            <span className="text-[0.625rem] font-medium px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
              blocklist financeiro aplicado
            </span>
          </div>
          <p className="text-xs text-[var(--nova-text-dim)] mb-4">Apenas parceiros sem débitos e com produção ≥ R$ 25 mil. Use os filtros para segmentar.</p>
              ...
              <div>
                ...
                {results.length > 0 && (
                  <p className="text-xs text-[var(--nova-text-dim)] mt-0.5">
                    Base elegível (sem débito) — telefone + 4 variáveis do template Meta
                  </p>
                )}
              </div>
              {results.length > 0 && (
